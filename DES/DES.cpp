...
int main()
{
	...
	message = "Welcome to DES Program";
	...
	str1 = d1.Encrypt(str);
	cout<<endl;
	message = "Plain Text read from input.txt is :";
	for (int x = 0; x < message.length(); x++)
	{
		cout << message.at(x);
		Sleep(10);
	...
	ifstream fin;
	fin.open("encryption.des");
	fout.open("encryption.des", ios::app); // Append mode
	if (fin.is_open())
		fout << str1; // Writing data to file
	fin.close();
	fout.close();
	string decrypt_str = d2.Decrypt(str1);
	message = "Decrypted text to be appended in output.txt is :";
	for (int x = 0; x < message.length(); x++)
	{
		cout << message.at(x);
		Sleep(10);
	...
	Sleep(1000);
	cout<<endl<<decrypt_str<<endl<<endl;
	getch();
	fstream f;
